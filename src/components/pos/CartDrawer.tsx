...
import { ShoppingCart, Plus, Minus, Trash2, Send, MessageSquare, User } from 'lucide-react';
...
interface CartDrawerProps {
  ...
  onPlaceOrder: (tableNumber: string, customerName: string, notes: string) => void;
}
...
export const CartDrawer = ({ items, onUpdateQuantity, onRemove, onPlaceOrder }: CartDrawerProps) => {
  ...
  const handlePlaceOrder = () => {
    ...
    onPlaceOrder(tableNumber, customerName, orderNotes);
    setTableNumber('');
    setCustomerName('');
    setOrderNotes('');
    setIsOpen(false);
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-3">
            {items.map((item) => (
              <div key={item.menuItem.id} className="flex items-center gap-3 p-3 bg-secondary/50 rounded-xl">
                ...
                <div className={`w-4 h-4 border-2 flex items-center justify-center rounded-sm shrink-0 ${item.menuItem.isVeg ? 'border-green-600' : 'border-red-600'
                  }`}>
